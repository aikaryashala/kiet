...
give two functions addresses of your own. The line <code>@route("/hai")</code> above
...
p("""The doors from Stage 3 map to pieces of Bottle. A path parameter, <code>&lt;name&gt;</code> in the
route, becomes a function argument — <code>def wish(name):</code> receives whatever was in the URL. The
query string is <code>request.query.get("k")</code>; that is for Stage 5. Today one route takes a name
from the path and the other takes nothing at all.""") + \
...
Good morning Lakshmi"""),
    p("<strong>Task 2.</strong> Fill in <code>about</code>: a dict with exactly the three keys of the <code>students</code> table, holding <em>your</em> details, typed in. This is the first JSON your own server sends, and it has the same shape as one row of Stage 1.") +
...
PASS: Task 2: /about has exactly the three keys of the students table
PASS: Task 2: /about values are non-empty strings
...
qz = quiz([
    ...
    ("A route returns <code>{\"inter_city\": \"Visakhapatnam\", \"ok\": True}</code>. What does curl print?",
     ["<code>{'inter_city': 'Visakhapatnam', 'ok': True}</code>", "<code>{\"inter_city\": \"Visakhapatnam\", \"ok\": true}</code>", "<code>inter_city=Visakhapatnam ok=True</code>", "an error, dicts cannot be returned"], 2,
     "Bottle runs json.dumps on the dict: double quotes, lowercase true. That is JSON, not Python."),
    ("<code>curl -i localhost:8081/about</code> shows <code>Content-Type: application/json</code>; <code>curl -i localhost:8081/wish/Ravi</code> shows <code>text/html</code>. Why?",
     ["about() was written second", "about() returns a dict, wish() returns a string", "Port 8081 sends JSON only for /about", "The name has letters in it"], 2,
     "Bottle sets Content-Type from what the function returns: a dict becomes JSON, a string is sent as text."),
