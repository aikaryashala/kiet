...
(function () {
  ...
  function hintFor(c) {
    ...
    if (/^git init\b/.test(c)) return "start tracking this folder with git: creates the hidden .git folder";
    if (/^git status\b/.test(c)) return "show which files changed since the last commit, and which are staged";
    if (/^git add \.$/.test(c)) return "stage every changed file in this folder for the next commit";
    if ((m = c.match(/^git add (.+)/))) return "stage " + m[1] + " for the next commit";
    if (/^git commit --amend/.test(c)) return "replace the last commit with a corrected one";
    if (/^git commit\b/.test(c)) return "save a snapshot of the staged files, with the message in quotes";
    if (/^git diff\b/.test(c)) return "show the exact lines that changed since the last commit";
    if (/^git log --oneline/.test(c)) return "list the commits, one short line each";
    if (/^git log\b/.test(c)) return "list the commits, newest first, with author, date and message";
    if (/^git push\b/.test(c)) return "send your commits to GitHub";
    if ((m = c.match(/^git checkout -b (\S+)/))) return "create the branch " + m[1] + " and switch to it";
    if ((m = c.match(/^git checkout (\S+) -- (\S+)/))) return "restore " + m[2] + " as it was in commit " + m[1] + "; other files untouched";
    if ((m = c.match(/^git checkout (\S+)/))) return "switch to " + m[1];
    if (/^git reset --soft/.test(c)) return "undo the commit, keep its changes staged";
    if (/^git reset --mixed/.test(c)) return "undo the commit, keep its changes in the files, unstaged";
    if (/^git reset --hard/.test(c)) return "undo the commit AND throw its changes away — no undo for this";
    if (/^git revert\b/.test(c)) return "add a new commit that reverses that commit; history is kept";
    if (/^git merge --abort/.test(c)) return "cancel the merge and go back to before git pull";
    if (/^git remote add origin/.test(c)) return "tell git where the GitHub copy of this project lives";
    if (/^git branch -M main/.test(c)) return "name the current branch main";
    if (/^git restore\b/.test(c)) return "throw away the changes in that file, back to the last commit";
    if (/^git show\b/.test(c)) return "show what one commit changed";
    if (/^cd ~$/.test(c)) return "go to your home folder";
    if (/^cd \.\.\/\.\.$/.test(c)) return "go up two folders";
    if (/^cd \.\.$/.test(c)) return "go up one folder, to the parent";
    if ((m = c.match(/^cd (\S+)$/))) return "go into the folder " + m[1];
    if (/^pwd$/.test(c)) return "print the folder you are in right now";
    if (/^ls -la?$/.test(c) || /^ls -a$/.test(c)) return "list this folder in detail, hidden files included";
    if (/^ls -l$/.test(c)) return "list this folder with sizes, dates and permissions";
    if ((m = c.match(/^ls ?(\S*)$/))) return "list what is inside " + (m[1] || "this folder");
    if (/^mkdir -p /.test(c)) return "create the folder and every parent folder on the way";
    if (/^mkdir /.test(c)) return "create the folder(s) named";
    if ((m = c.match(/^mv (.+) (\S+)\/$/))) return "move " + m[1] + " into the folder " + m[2];
    if (/^mv -i /.test(c)) return "move, but ask before overwriting anything";
    if ((m = c.match(/^mv (\S+) (\S+)$/))) return "rename or move " + m[1] + " to " + m[2];
    if (/^touch /.test(c)) return "create the empty file(s) named";
    if (/^code \.$/.test(c)) return "open this folder in VS Code";
    if (/^sudo apt install/.test(c)) return "install that package system-wide; sudo asks for your password";
    if (/^sudo apt update/.test(c)) return "refresh the list of packages apt knows about";
    if (/^clear$/.test(c)) return "clear the terminal screen";
